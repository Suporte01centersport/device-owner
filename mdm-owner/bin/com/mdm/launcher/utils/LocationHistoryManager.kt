...
object LocationHistoryManager {
    ...
    fun saveLocation(context: Context, location: Location, address: String? = null) {
        synchronized(lock) {
            try {
                ...
                Log.d(TAG, "📍 Localização salva no histórico: ${entry.latitude}, ${entry.longitude} (${trimmedHistory.size} entradas)")
            } catch (e: Exception) {
    ...
    fun getLocationStats(context: Context): Map<String, Any> {
        ...
        return mapOf(
            ...
            "last_location" to (history.maxByOrNull { it.timestamp }?.let { formatLocationEntry(it) } ?: ""),
