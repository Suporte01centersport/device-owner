 ...
 * Reinicia o dispositivo (fallback HTTP quando WebSocket não conectado)
 */
export async function POST(
  ...
  try {
    ...
    const url = `http://${wsHost}:${wsPort}/api/devices/${deviceId}/reboot`
    ...
    if (res.ok && data.success !== false) {
      return NextResponse.json({ success: true, message: 'Comando de reinicialização enviado' })
    }
